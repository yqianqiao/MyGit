...
public abstract class BaseActivity extends RxAppCompatActivity {
    ...
    protected SlidrInterface slidrInterface;//侧滑退出
    private int iconType = -1;

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        iconType = SettingUtil.getInstance().getCustomIconValue();
        initSlidable();
    }
     ...
     * @param homeAsUpEnableb 给左上角图标的左边加上一个返回的图标
     * @param title           标题
     */
    protected void initToolBar(Toolbar toolbar, boolean homeAsUpEnableb, String title) {
        toolbar.setTitle(title);
        setSupportActionBar(toolbar);
        getSupportActionBar().setDisplayHomeAsUpEnabled(homeAsUpEnableb);
    }
